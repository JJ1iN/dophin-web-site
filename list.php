...
<div class="container">
  ...
  <table class="table">
    ...
    <tbody>
    <?php
      if($num != 0) { # If posts exist
        for ( $i=0; $i<$num; $i++ ) {
          $row = $result->fetch_assoc();?>
          <tr>
            <th scope="row" class="text-center"><?=$row["idx"]?></th>
            <?php if($row["secret"]=="y") { ?>
              <td style="text-align:center;"><span style="display:inline-block; height:15px; width:15px;" data-feather="lock"></span>&nbsp;<a href="index.php?page=auth&idx=<?=$row["idx"]?>&mode=view"><?=$row["title"]?></a></td>
            <?php } else { ?>
              <td style="text-align:center;"><a href="index.php?page=view&idx=<?=$row["idx"]?>"><?=$row["title"]?></a></td>
            <?php } ?>
